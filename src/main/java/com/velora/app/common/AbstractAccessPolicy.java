 ...
 *
 * Declares the abstract {@code check()} method that each subclass must implement
 * with domain-specific role rules. Provides a concrete {@code requireAdmin()} method
 * that delegates to {@code check()} using the "ADMIN_ONLY" operation sentinel.
 *
 * Subclasses: NotificationAccessPolicy, FeedbackAccessPolicy, RolePolicy, AnalyticsAccessPolicy
 *
...
public abstract class AbstractAccessPolicy {
    ...
    /**
     * Checks whether the given actor role is permitted to perform the specified operation.
     *
